/*
* Polynomial Addition and Reverse Printing using Linked Lists
* Description: Reads two polynomials from files, adds them, prints the result normally and in reverse order.
*/
...
void printreverse(struct num *tp)
//this function is to print the poly from the last node using recursion
{
	tp=tp->next; //as ussual, we will begin (end) on the second node
	
	if(tp->next==NULL)
	//this statement will only work when the linked list is ended (last number on poly)
	{
		printf(" %dx^%d ", tp->number, tp->power);
	}
	else
	//this statement will bring us to the last number and printing it first using recursion.
	{
		printreverse(tp);
		if(tp->number>0)
		{printf("+");}
